...
function buildHSMonthlyCohortFromVault_(academicSnapshotRows) {
  ...
  try {const currentMonthKey = _normMonthLabel(Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'MMM yyyy'));
    const byStudentRaw = {};
    ...
    const weeklyGainByStudentMonth = {}; // studentId -> monthKey -> { sum, count }
    academicSnapshotRows.forEach(row => {
      if (String(row.cadence || '').trim().toLowerCase() !== 'weekly') return;
      const id = String(row.studentId || '').trim();
      if (!id) return;
      const gain = row.gain !== undefined && row.gain !== '' ? Number(row.gain) : null;
      if (gain === null || isNaN(gain)) return; // skip the weekly row's own 'New'/blank weeks
      const dateStr = String(row.snapshotDate || '');
      const dateObj = _parseLocalDate(dateStr.slice(0, 10)) || new Date(dateStr);
      if (isNaN(dateObj.getTime())) return;
      const monthKey = _normMonthLabel(Utilities.formatDate(dateObj, Session.getScriptTimeZone(), 'MMM yyyy'));
      if (!weeklyGainByStudentMonth[id]) weeklyGainByStudentMonth[id] = {};
      if (!weeklyGainByStudentMonth[id][monthKey]) weeklyGainByStudentMonth[id][monthKey] = { sum: 0, count: 0 };
      weeklyGainByStudentMonth[id][monthKey].sum   += gain;
      weeklyGainByStudentMonth[id][monthKey].count += 1;
    });
    const finalByStudent = {};
    ...
    Object.entries(byStudentRaw).forEach(([id, rows]) => {
      ...
      const months = rows.map(row => {
        ...
        let creditsGained = null;
        let estimatedFromWeekly = false;
        if (!inProgress) {
          if (!isNew) {
            creditsGained = _toNumber(row.gain);
          } else {
            const weeklyFallback = weeklyGainByStudentMonth[id]?.[monthKey];
            if (weeklyFallback && weeklyFallback.count > 0) {
              creditsGained = +weeklyFallback.sum.toFixed(2);
        ...
        return { month: monthLabel, monthKey, snapshotDate: dateStr, startCredits, endCredits, creditsGained, inProgress, estimatedFromWeekly };
      });
    ...
    const cohort = Object.entries(monthEntries).map(([monthKey, entries]) => {
      ...
      const avgGained    = gains.length ? +(gains.reduce((a, v) => a + v, 0) / gains.length).toFixed(1) : null;
      const totalGained  = +gains.reduce((a, v) => a + v, 0).toFixed(1);
      // K — of those N, how many actually reduced credits (gain > 0)
      const reducedCount = completed.filter(e => e.creditsGained > 0).length;
      return {
        month:          entries[0] ? entries[0].month : monthKey,
        studentCount:   entries.length,   // total tracked this month, regardless of whether a gain was computable
        completedCount: completed.length, // N — the average's actual denominator; see note above
        reducedCount,                     // K — of N, how many reduced credits
        avgGained,
        totalGained,
      };
    }).filter(m => m.studentCount > 0)
    ...
    return { byStudent: finalByStudent, cohort };
  } catch(e) {Logger.log('buildHSMonthlyCohortFromVault_ error: ' + e.message); return empty;}
}
