  ...
  try {
    ...
    transcriptRows
      ...
      .forEach(r => {
        ...
        if (baseName === 'Trade Completion') return;
        ...
        if (entry) {
          ...
          matchedCount++;
        } else {unmatchedCourses.push(r.courseName);}
      });
...
function autoPopulateTranscript(studentId, employeeId, role) {
  ...
  try {
    ...
    if (existingRows.length > 0) {return { success: true, skipped: true, message: 'This student already has transcript rows — auto-populate only applies to new, empty transcripts.' };}
