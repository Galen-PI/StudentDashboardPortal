 ...
 *
 *   runWeeklyWIRGenerationV2(weekLabel) is the weekly entry point,
 *   writing through the existing WIRDataLayer.gs storage layer
 *   unchanged. generateWeeklyWIR() (the client-facing wrapper) now
 *   calls it directly — this IS the live production path.
 *
 * STILL TO VERIFY:
...
function runWeeklyWIRGenerationV2(weekLabel) {
  ...
  studentIds.forEach((studentId, idx) => {
    ...
    pendingRows.push({
      ...
      streak: '',
      trajectory: '',
      gradGap: _formatGradGap_(_computeGradGap_(s)),
