...
function appendWIRReportRows(rowObjects) {
  ...
  // Locked: this is a read-existing-keys-then-append dedupe pattern.
  // Without a lock, an overlapping batch generation run and a single-
  // student preview/save (or two overlapping batch runs) can each read
  // the same "existing keys" snapshot, miss each other's in-flight
  // write, and both append — producing duplicate WIR report rows for
  // the same student+week instead of the intended dedupe. _withWirLock_
  // is already used elsewhere in this file (saveCaseManagement) for the
  // same reason.
