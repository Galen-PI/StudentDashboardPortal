...
function getProductivityPayloadFromVault_() {
  try {
    ...
    _ensureProductivityExclusionsSheet_();
    const exclusionRows      = readVaultSheetAsObjects_(VAULT_SHEET_PRODUCTIVITY_EXCLUSIONS, VAULT_PRODUCTIVITY_EXCLUSIONS_HEADERS);
    const exclusionByKey     = {};
    exclusionRows.forEach(r => {
      const sid = String(r.studentId || '').trim();
      const wk  = _normVaultDateField_(r.weekLabel, 'yyyy-MM-dd');
      if (sid && wk) exclusionByKey[sid + '||' + wk] = String(r.reason || '').trim() || 'Excluded';
    });
    const productivityData   = _buildProductivityDataFromVault_(productivityRows, nameById, exclusionByKey);
...
function _buildProductivityDataFromVault_(productivityRows, nameById, exclusionByKey) {
  ...
  Object.entries(byStudent).forEach(([sid, rows]) => {
    ...
    rows.forEach(row => {
      ...
      const weekLabel = _normVaultDateField_(row.weekLabel, 'yyyy-MM-dd');
      const exclusionReason = exclusionByKey[sid + '||' + weekLabel];
      ...
      // Excluded weeks stay visible in this student's own week list
      // (tagged, above) but don't feed totals/trend/peak-trough —
      // that's the whole point of marking one.
    ...
    const eligibleWeeks = weeks.filter(w => !w.excluded);
    const sorted     = [...eligibleWeeks].filter(w => w.productivity !== null).sort((a, b) => b.productivity - a.productivity);
    const peakWeek   = sorted[0]                 || null;
    const troughWeek = sorted[sorted.length - 1] || null;

    const half      = Math.floor(eligibleWeeks.length / 2);
    const firstHalf = eligibleWeeks.slice(0, half);
    const secHalf   = eligibleWeeks.slice(half);
    ...
    results.push({
      ...
      troughWeek,
      trend,
      weeksWithData:       eligibleWeeks.length,
    });
  });
...
function _buildProductivityCohortSummary(productivityData) {
  ...
  productivityData.forEach(student => {
    student.weeks.forEach(w => {
      if (w.excluded) return; // audited out — see setProductivityExclusion
      if (!weekMap[w.label]) {
  ...
  // Sorted explicitly by label (ISO 'yyyy-MM-dd', already normalized
  // above) — Object.values(weekMap) on its own only reflects
  // whichever order each week-label was first encountered while
  // looping student-by-student, NOT chronological order. A student
  // whose data starts later in the year could introduce an earlier
  // week-label the map hadn't seen yet, which would otherwise get
  // appended at the end of the array instead of sorted into its
...
// Wraps _computeHSGraduationPredictions (Predictions.gs) for the
// client — pulls profiles + HS monthly cohort data from the current
// dashboard payload and returns ranked predictions plus the
// month/weeks-left context the UI displays alongside them.
...
// ── Trade Completion Report (current % + end-of-month projection) ──
function getTradeCompletionReport() {
  try {
    const dashboard = getDashboardData();
    if (dashboard.error) return { error: dashboard.error, students: [] };

    const today   = new Date();
    const monthEnd = new Date(today.getFullYear(), today.getMonth() + 1, 0);
    const daysLeftInMonth = Math.max(Math.round((monthEnd - today) / 86400000), 0);
    const weeksLeftInMonth = +(daysLeftInMonth / 7).toFixed(2);

    const rows = [];
    (dashboard.profiles || []).forEach(p => {
      if (!p.trades || !p.trades.length) return;
      p.trades.forEach(t => {
        if (t.overallPct === null || t.overallPct === undefined) return;
        if (t.overallPct >= 100) return; // already complete — nothing left to project

        const trend = t.tradeTrend || null;
        const weeklyRate = trend && trend.effectiveRatePerWeek !== null ? trend.effectiveRatePerWeek : null;

        let projectedPct = null;
        if (weeklyRate !== null) {
          const raw = t.overallPct + (weeklyRate * weeksLeftInMonth);
          projectedPct = +Math.min(100, Math.max(0, raw)).toFixed(1);
        }

        rows.push({
          id:                 p.id,
          name:               p.displayName,
          trade:              t.tarName || '(unnamed trade)',
          overallPct:         t.overallPct,
          weeklyRate,
          trendLabel:         trend ? trend.trendLabel : null,
          accelerationFactor: trend ? trend.accelerationFactor : null,
          overrideMultiplier: trend ? trend.overrideMultiplier : null,
          projectedPct,
          status:             t.status || null,
          etarProjectedEnd:   t.etarProjectedEnd || null,
          daysToETAR:         t.daysToETAR !== undefined ? t.daysToETAR : null,
        });
      });
    });
    ...
    Logger.log('getTradeCompletionReport error: ' + e.message);
